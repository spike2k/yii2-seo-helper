...
class Meta extends Component {
    ...
    public function set($model,$useOnlySEO = false)
    {
        ...
        if(!$useOnlySEO){
            ...
            if(!$image)
                $image = (isset($model->banner)?$model->banner:(isset($model->baner)?$model->baner:(isset($model->thumb)?$model->thumb:(isset($model->foto)?$model->foto:null))));
    ...
    protected function _applyOGs($event)
    {
        $this->_view = $event->sender;

        if(empty($this->tags["og:description"])) {
    ...
    protected function _applyMeta($event)
    {
        ...
        if(empty($data["og:description"])) $data['tags']["og:description"] = $data['metadesc'];
        if(empty($data["og:title"])) $data['tags']["og:title"] = $data['title'];
